...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT * FROM airplane WHERE (origin = ? OR ? IS NULL OR ? = '') AND (destination = ? OR ? IS NULL OR ? = '') AND (arrivaltime = ? OR ? IS NULL OR ? = '') AND (departuretime = ? OR ? IS NULL OR ? = '') AND (passengerleft >= ? OR ? IS NULL OR ? = '')");
