...
if ($result->num_rows > 0) {
    $deleteStmt = $conn->prepare("DELETE FROM train WHERE id = ? AND passengerleft = ?");
...
if ($result->num_rows > 0) {
    ...
    $updateStmt = $conn->prepare("UPDATE train SET passengerleft = ? WHERE id = ?");
...
//insert reservation to trainresevrvation 
$ids=0;
// Get the highest ID from the database
$result = $conn->query("SELECT MAX(id) AS max_id FROM train_reservation");
$row = $result->fetch_assoc();
...
$ids = $highestId + 1;
 ...
 $stmt = $conn->prepare("INSERT INTO train_reservation (firstname, lastname, nationalcode,birthdate,gender,id,trainid) VALUES (?, ?, ?,?,?,?,?)");
