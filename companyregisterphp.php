...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!preg_match("/^\d{11}$/", $phoneNumber)) {
        ...
        $passwordMatch = "Passwords do not match";
          ...
          $stmt = $conn->prepare("SELECT * FROM company_registration WHERE phoneNumber = ?");
          $stmt->bind_param("s", $phoneNumber);
          $stmt->execute();
          $result = $stmt->get_result();
  
             // Check if user with the given phone number already exists
             $stmt = $conn->prepare("SELECT * FROM company_registration WHERE email = ?");
...
$result = $conn->query("SELECT MAX(id) AS max_id FROM company_registration");
...
$id = $highestId + 1;
        ...
        $stmt = $conn->prepare("INSERT INTO company_registration (phoneNumber, email, password,id) VALUES (?, ?, ?,?)");
